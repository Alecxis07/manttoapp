...
const deactivate = () => {
    if (confirm(`¿Desactivar el concepto ${props.part.code}?`)) {
        ...
        <template #header>
            <div class="flex items-center justify-between">
                ...
                <div class="flex gap-2">
                    ...
                    <SecondaryButton v-if="part.is_active" @click="deactivate">
                        Desactivar
                    </SecondaryButton>
                </div>
            </div>
                ...
                <div class="bg-white shadow-xl sm:rounded-lg p-6 space-y-4">
                    <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <dd class="text-sm text-gray-900">{{ part.description }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm text-gray-500">Tipo</dt>
                            <dd class="text-sm text-gray-900">{{ part.type_label }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm text-gray-500">Categoría</dt>
                            <dd class="text-sm text-gray-900">{{ part.category?.name ?? '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm text-gray-500">Precio base</dt>
                            <dd class="text-sm text-gray-900">${{ part.base_price }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm text-gray-500">Unidad</dt>
                            <dd class="text-sm text-gray-900">{{ part.unit_of_measure }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm text-gray-500">Estatus</dt>
                            <dd class="text-sm text-gray-900">{{ part.is_active ? 'Activo' : 'Inactivo' }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm text-gray-500">En uso (RN-CAT-001)</dt>
                            <dd class="text-sm text-gray-900">{{ part.in_use ? 'Sí — solo desactivable' : 'No' }}</dd>
                        </div>
                    </dl>
                </div>
            </div>
